...
public class Shoot : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            {
                ZombieController zombieController = hitInfo.collider.GetComponentInParent<ZombieController>();
                if (zombieController != null)
                {
                    ...
                    Vector3 forceDirection = zombieController.transform.position - _camera.transform.position;
                    forceDirection.y = 1;
                    forceDirection.Normalize();
                    Vector3 force = forceMagnitude * forceDirection;
                    zombieController.TriggerRagdoll(force, hitInfo.point);
                }
            }
        }
    }
